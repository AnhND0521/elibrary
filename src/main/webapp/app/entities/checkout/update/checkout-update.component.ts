...
export class CheckoutUpdateComponent implements OnInit {
  ...
  save(): void {
    ...
    if (checkout.id !== null && checkout.id !== 0) {
      this.subscribeToSaveResponse(this.checkoutService.update(checkout));
    } else {
      this.subscribeToSaveResponse(this.checkoutService.create({ ...checkout, id: null }));
    }
  }
  ...
  protected loadRelationshipsOptions(): void {
    this.bookCopyService
      ...
      .pipe(
        map((bookCopies: IBookCopy[]) => this.bookCopyService.addBookCopyToCollectionIfMissing<IBookCopy>(bookCopies, this.checkout?.copy))
      )
    ...
    this.patronAccountService
      ...
      .pipe(
        map((patronAccounts: IPatronAccount[]) =>
          this.patronAccountService.addPatronAccountToCollectionIfMissing<IPatronAccount>(patronAccounts, this.checkout?.patron)
        )
      )
